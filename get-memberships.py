...
@click.command()
@click.option(
    "--group", "-g", required=False, help="Name of group (including subgroups) to check"
)
@click.option(
    "--group-id",
    "-i",
    required=False,
    help="ID of group (including subgroups) to check",
)
@click.option(
    "--user",
    "-u",
    default=None,
    help="[Optional] Name of user to look up, omit for all users",
)
@click.option(
    "--shared-group",
    "-s",
    default=None,
    help="[Optional] Name of a group that has access granted, omit for all groups",
)
@click.option(
    "--url",
    "-U",
    envvar="GITLAB_URL",
    help="Gitlab url (can be set with environment variable GITLAB_URL)",
    required=True,
)
@click.option(
    "--token",
    "-t",
    envvar="GITLAB_TOKEN",
    help="Gitlab token (can be set with environment variable GITLAB_TOKEN)",
    required=True,
)
@click.option(
    "--json-output",
...
def get_memberships(
    group, group_id, user, shared_group, url, token, json_output, verbose
):
    ...
    if group_id and group:
        print(
            "Both group and group-id is passed, this redundant, and group-id will take prevalence.",
            file=print_outputs,
        )

    base_group = None

    if not (group_id or group):
        raise click.UsageError("Either --group or --group-id must be provided")
    ...
    if group_id:
        ...
        try:
            ...
            if len(base_groups) > 1:
                print(
                    f"Multiple groups with name {group} are found, the group name must be unique!",
                    file=print_outputs,
                )
                if verbose:
                    for g in base_groups:
                        print(
                            f"\t{g.attributes['name']} with id {g.attributes['id']}",
                            file=sys.stderr,
                        )
                sys.exit(1)
            ...
            if verbose:
                print(
                    f"(Single) Group {group} with id {base_group.attributes['id']} found!\n",
                    file=sys.stderr,
                )
    ...
    for group in groups_in_scope:
        ...
        for p in group.projects.list(all=True):
            try:
                ...
                project_name = p.attributes.get(
                    "path_with_namespace"
                ) or p.attributes.get("name", "unknown")
                print(
                    f"Failed to fetch project {project_name}: {e.error_message} (status {e.response_code}). Skipping.",
                    file=sys.stderr,
                )
                continue
    ...
    for key in memberships:
        ...
        if memberships[key]:
            if not (user or shared_group):
                print(
                    f"{t} {g['name']} ({path}) with id {g['id']} has the following memberships ==>",
                    file=print_outputs,
                )
            for m in memberships[key]:
                ...
                if (user and user == name) or (shared_group and shared_group == name):
                    print(
                        f"{t} {g['name']} with id {g['id']} has a matching membership ==>",
                        file=print_outputs,
                    )
            ...
            print(
                f"{t} {g['name']} with id {g['id']} does not have {'matching ' if user else ''}memberships",
                file=print_outputs,
            )
